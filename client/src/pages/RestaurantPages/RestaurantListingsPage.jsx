...
const RestaurantListingsPage = () => {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8800/listings/${restaurantId}`);
  ...
  const handleAddListing = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://127.0.0.1:8800/listings', newListing);
  ...
  const handleDeleteListing = async (id) => {
    ...
    try {
      await axios.delete(`http://127.0.0.1:8800/listings/${id}`);
